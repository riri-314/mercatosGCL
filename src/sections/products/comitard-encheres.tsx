...
const EncheresList: React.FC<EncheresListProps> = ({
  encheres,
  cerclesData,
  won,
}) => {
      ...
      <TableContainer component={Paper}>
        <Table>
          ...
          <TableHead>
            <TableRow>
              <TableCell>
                <Typography variant="subtitle1">Cercle enchérisseur</Typography>
              </TableCell>
              <TableCell>
                <Typography variant="subtitle1">Date</Typography>
              </TableCell>
              <TableCell>
                <Typography variant="subtitle1">Enchère</Typography>
              </TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {Object.entries(encheres)
              ...
              .map(([_id, encheresData], index) => (
                <TableRow
                  key={index}
                  sx={{
                    backgroundColor: (theme) =>
                      index === 0 ? `${theme.palette.info.light}` : "",
                  }}
                >
                  <TableCell>
                    <Stack direction="row" spacing={1}>
                      ...
                      <strong>
                        {cerclesData[encheresData.sender]?.name || "Erreur"}
                      </strong>
                    </Stack>
                  </TableCell>
                  <TableCell>
                    {encheresData.date.toDate().toLocaleString()}
                  </TableCell>
