...
export default function NewEdition({
                                       data, updateMode, close, refetchData,
                                   }: DataContextValue) {
    ...
    const [remboursementVendeur, setRemboursementVendeur] = useState<number>(updateMode && data.remboursementVendeur ? data.remboursementVendeur : 0.5);

    const [remboursementPerdant, setRemboursementPerdant] = useState<number>(updateMode && data.remboursementPerdant ? data.remboursementPerdant : 1);

    const [remboursementGagnant, setRemboursementGagnant] = useState<number>(updateMode && data.remboursementGagnant ? data.remboursementGagnant : 0);
    ...
    async function handleCreateEdition() {
        ...
        if (rules == "") {
            ...
            setError("La date de fin doit être supérieur à la date de début et la période les séparant doit être plus grande que la durée d'une enchere");
            setLoading(false);
            return;
        } else if (nbFut <= 0) {
            ...
            setError("L'enchère minimum doit être inférieur au nombre de futs par cercle");
            setEnchereMinError(true);
            setNbFutError(true);
            setLoading(false);
            return;
        } else if (remboursementVendeur < 0 || remboursementVendeur < 0 || remboursementVendeur < 0) {
        ...
        if (updateMode) {
            ...
            ret = await newEdition(rules, start, stop, nbFut, duration, nbComitard, enchereMin, enchereMax, remboursementVendeur, remboursementPerdant, remboursementGagnant);
        }
    ...
    return (<>
            <Grid container spacing={2}>
                ...
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <Grid item xs={12} sm={6}>
                        <DateTimePicker
                            ...
                            defaultValue={data.start && updateMode ? dayjs(data.start.toDate()) : null}
                    ...
                    <Grid item xs={12} sm={6}>
                        <DateTimePicker
                            ...
                            defaultValue={data.start && updateMode ? dayjs(data.stop.toDate()) : null}
                            sx={{width: "100%"}}
                        />
                    </Grid>
                ...
                <Grid item xs={12} sm={4}>
                    <QuantityInput
                        ...
                        helpText={`Nombre de futs qu'un cercle possède initialement. Défaut: ${data.nbFut ? data.nbFut : 100}`}
                        change={(_event: | React.FocusEvent<HTMLInputElement> | React.PointerEvent | React.KeyboardEvent, val: number | undefined) => {
                ...
                <Grid item xs={12} sm={4}>
                    <QuantityInput
                        ...
                        helpText={`Nombre maximum de comitard qu'un cercle peut proposer à l'enchère. Défaut:  ${data.nbComitard ? data.nbComitard : 5}`}
                        change={(_event: | React.FocusEvent<HTMLInputElement> | React.PointerEvent | React.KeyboardEvent, val: number | undefined) => {
                            if (typeof val !== "undefined") {
                                setNbComitard(val);
                ...
                <Grid item xs={12} sm={4}>
                    <QuantityInput
                        ...
                        helpText={`Durée d'une enchère en heures. Défaut: ${data.duration ? data.duration : 16}`}
                        change={(_event: | React.FocusEvent<HTMLInputElement> | React.PointerEvent | React.KeyboardEvent, val: number | undefined) => {
                            if (typeof val !== "undefined") {
                                setDuration(val);
                ...
                <Grid item xs={12} sm={6}>
                    <QuantityInput
                        ...
                        helpText={`Enchere minimum qu'il cercle peut mettre sur un comitard. Defaut: ${data.enchereMin ? data.enchereMin : 1}`}
                        change={(_event: | React.FocusEvent<HTMLInputElement> | React.PointerEvent | React.KeyboardEvent, val: number | undefined) => {
                            if (typeof val !== "undefined") {
                                setEnchereMin(val);
                ...
                <Grid item xs={12} sm={6}>
                    <QuantityInput
                        ...
                        helpText={`Enchere maximum qu'il cercle peut mettre sur un comitard. Defaut ${data.enchereMax ? data.enchereMax : 100}`}
                        change={(_event: | React.FocusEvent<HTMLInputElement> | React.PointerEvent | React.KeyboardEvent, val: number | undefined) => {
                            if (typeof val !== "undefined") {
                                setEnchereMax(val);
                ...
                <Grid item xs={12} sm={4}>
                    <UnstyledSelectIntroduction
                        ...
                        helpText={`Pourcentage de la somme des futs mis en enchère qui est remboursée au cercle vendant son comitard. Défaut: ${data.remboursementVendeur ? data.remboursementVendeur * 100 : 50}%`}
                ...
                <Grid item xs={12} sm={4}>
                    <UnstyledSelectIntroduction
                        ...
                        helpText={`Pourcentage de la somme des futs mis en enchère qui est remboursé au cercle perdant l'enchère. Défaut: ${data.remboursementPerdant ? data.remboursementPerdant * 100 : 100}%`}
                ...
                <Grid item xs={12} sm={4}>
                    <UnstyledSelectIntroduction
                        ...
                        helpText={`Pourcentage de la somme des futs mis en enchère qui est remboursé au cercle gagnant l'enchère. Défaut: ${data.remboursementGagnant ? data.remboursementGagnant * 100 : 0}%`}
                        change={(_event: any, val: any) => setRemboursementGagnant(val)}
                    />
                </Grid>
                {updateMode && (<Grid item xs={12} sm={12}>
                        <LoadingButton
                            size="large"
                            variant="contained"
                            color="error"
            ...
            {error && (<Alert sx={{mt: 3}} severity={errorType}>
                    {error}
                </Alert>)}
        </>);
}
