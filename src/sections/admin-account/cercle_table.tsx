...
export default function CercleTable({
                                        data, refetchData, error, handleOpenModalCercle,
                                    }: CercleTableProps) {
    ...
    const cercleDataArray = Object.keys(data.data().cercles).map((id: string) => ({id, ...data.data().cercles[id]}));
    ...
    const columns: GridColDef[] = [{field: "id", headerName: "ID", width: 90}, {
        field: "description", headerName: "Déscription", minWidth: 200, editable: false,
    }, {
        field: "name", headerName: "Nom", minWidth: 150, editable: false,
    }, {
        field: "nbFut", headerName: "Nombre de futs restants", type: "number", minWidth: 150, editable: false,
    }, {
        field: "actions",
        type: "actions",
        headerName: "Éditer",
    ...
    return (<>
            ...
            <WarningModal loading={loading} title="Attention!" message={modalData[2]} open={openModal}
                          close={() => setOpenModal(false)} onProceed={() => handleClick(modalData[0], modalData[1])}/>
        </>);
}
